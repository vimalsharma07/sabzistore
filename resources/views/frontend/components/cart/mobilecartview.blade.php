...
<div class="cart-layout">
                ...
                @if($product)
                    ...
                    <div class="product-right">
                        <div class="product-quantity">
                            <span id="decreaseQty{{$product->id}}" class="decreaseQty" data-id="{{$product->id}}"  data-key="{{array_key_first($item['attributes'])}}"  data-value="{{ array_values($item['attributes'])[0]}}" >-</span>
                            <span class="quantity-value" id="quantity{{$product->id}}" data-id="{{$product->id}}">{{ $item['quantity'] }}</span>
                            <span id="increaseQty{{$product->id}}"  class="increaseQty selected"  data-id="{{$product->id}}"  data-key="{{array_key_first($item['attributes'])}}"  data-value="{{ array_values($item['attributes'])[0]}}">+</span>
                        </div>
                    </div>
                @else
    ...
    <div class="address-selection">
        ...
        @if($address!='')
           ...
           @else
           <div>
            <button  class="btn btn-primart" data-bs-toggle="modal" data-bs-target="#otpModal" aria-expanded="false"> Log In To proccedd</button>
           </div>
           @endif
